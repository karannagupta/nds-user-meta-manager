...
class User_List_Table extends Libraries\WP_List_Table  {
	...
	protected function column_user_login( $item ) {
		...
		$query_args_view_usermeta = array(
			...
			'action'	=> 'nds_view_user_meta',
			'nds_user'		=> absint( $item['ID']),
			'_wpnonce'	=> wp_create_nonce( 'nds_view_user_meta' ),
		);
		$view_usermeta_link = esc_url( add_query_arg( $query_args_view_usermeta, $admin_page_url ) );		
		$actions['nds_view_user_meta'] = '<a href="' . $view_usermeta_link . '">' . __( 'View Meta', $this->plugin_text_domain ) . '</a>';		
				
		// row actions to add usermeta.
		$query_args_add_usermeta = array(
			'page'		=>  wp_unslash( $_REQUEST['page'] ),
			'action'	=> 'nds_add_user_meta',
			'nds_user'		=> absint( $item['ID']),
			'_wpnonce'	=> wp_create_nonce( 'nds_add_user_meta' ),
		);
		$add_usermeta_link = esc_url( add_query_arg( $query_args_add_usermeta, $admin_page_url ) );		
		$actions['nds_add_user_meta'] = '<a href="' . $add_usermeta_link . '">' . __( 'Add  Meta', $this->plugin_text_domain ) . '</a>';		
		
		// row actions to delete usermeta.
		$query_args_delete_usermeta = array(
			'page'		=>  wp_unslash( $_REQUEST['page'] ),
			'action'	=> 'nds_delete_user_meta',
			'nds_user'		=> absint( $item['ID']),
			'_wpnonce'	=> wp_create_nonce( 'nds_delete_user_meta' ),
		);
		$delete_usermeta_link = esc_url( add_query_arg( $query_args_delete_usermeta, $admin_page_url ) );		
		$actions['nds_delete_user_meta'] = '<a href="' . $delete_usermeta_link . '">' . __( 'Delete  Meta', $this->plugin_text_domain ) . '</a>';		
	...
	public function handle_table_actions() {
		...
		$the_table_action = $this->current_action();
			...
			if ( ! wp_verify_nonce( $nonce, 'nds_view_user_meta' ) ) {
				...
				$this->nds_view_user_meta( absint( $_GET['nds_user']), $this->plugin_text_domain );
				$this->graceful_exit();
			}
		}
		...
		if ( 'nds_add_user_meta' === $this->current_action() ) { 
			...
			if ( ! wp_verify_nonce( $nonce, 'nds_add_user_meta' ) ) {
				...
				$this->nds_add_user_meta( absint( $_GET['nds_user']), $this->plugin_text_domain );
				$this->graceful_exit();
			}
		}
